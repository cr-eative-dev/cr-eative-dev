...
const BP_MEDIUM = 550;
const BP_LARGE = 700;
const BODY_COPY = `Software Developer and 3D Artist blending design with technology to craft innovative solutions.`;
...
export const shared = /* css */ `
	...
	*::after {
	...
	.wrapper {
		...
		font-family: -apple-system,BlinkMacSystemFont,"Courier","Segoe UI","Noto Sans",Helvetica,Arial,sans-serif,"Apple Color Emoji","Segoe UI Emoji";
...
export const top = (props: Props & { contributions: number }) => {
  ...
  const html = /*html*/ `
		<div class="wrapper grid label">
			<div class="menu fade-in">Ctrl+Click:</div>
...
export const fallback = (props: Props & { width: number }) => {
  ...
  const html = /* html */ `
		<main class="wrapper">
          ...
          .map((c, i) => `<span class="fade-in" style="--i: ${i};">${c}</span>`)
          .join('')}</p>
				<p class="hint fade-in">— Please try Chrome/Safari for this one! Firefox is not supported.</p>
			</div>
		</main>
	`;
